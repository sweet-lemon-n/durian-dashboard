...
function matchKey(containerNo, value) {
  const c = String(containerNo || '').trim();
  const d = dateKey(value);
  return c && d ? `${c}|${d}` : '';
}

function isOverseasTemperature(item) {
  const text = `${item.transitStatus || ''} ${item.statusText || ''} ${item.location || ''} ${item.note || ''}`;
...
function normalizeShipment(item) {
  return {
    ...
    temperatureStatusText: item.temperatureStatusText || '',
    missingTemperature: !!item.missingTemperature,
...
function normalizeTemperature(item) {
  ...
  const status = rawStatus === 'ALARM'
    ? rawStatus
    : deviation > TEMP_WARNING_DEVIATION
      ? 'ALARM'
      : deviation > TEMP_OK_DEVIATION
        ? 'WARN'
        : 'OK';
  return {
    containerNo: firstContainer(item),
    ...
    location: item.location || item.currentLocation || item.position || item.port || '',
    port: item.port || '',
    releaseDate: item.releaseDateText || item.placementTimeText || item.releaseTimeText || item.releaseDate || item.placementTime || item.releaseTime || '',
    recordedAt: item.recordedAtText || item.timeText || item.dateText || item.recordedAt || item.time || item.date || '',
    note: item.note || '',
    transitStatus: item.transitStatus || item.stage || '',
  };
}

function temperatureMap(temperature) {
  const byContainer = new Map();
  const byShipmentDate = new Map();
  (temperature.details || []).forEach(item => {
    if (item.containerNo && !byContainer.has(item.containerNo)) byContainer.set(item.containerNo, item);
    const key = matchKey(item.containerNo, item.releaseDate);
    if (key) {
      if (!byShipmentDate.has(key)) byShipmentDate.set(key, []);
      byShipmentDate.get(key).push(item);
    }
  });
  byShipmentDate.forEach(items => {
    items.sort((a, b) => (parseTimeMs(a.recordedAt) || 0) - (parseTimeMs(b.recordedAt) || 0));
  });
  return { byContainer, byShipmentDate };
}

function temperaturesForShipment(temperatureIndex, shipment) {
  const key = matchKey(firstContainer(shipment), shipment && shipment.shipDate);
  if (key && temperatureIndex.byShipmentDate.has(key)) return temperatureIndex.byShipmentDate.get(key);
  return [];
}

function temperatureForShipment(temperatureIndex, shipment) {
  const records = temperaturesForShipment(temperatureIndex, shipment);
  return records[records.length - 1] || null;
}

function locationForShipment(item, temp) {
  if (item.status === 'overseasTransit') {
    return firstText(temp && temp.location, item.location, item.currentLocation, item.port);
  }
  if (item.status === 'onShore') {
    return firstText(
      item.customsPort,
      item.port,
      item.arrivalPort,
  ...
  return firstText(item.location, item.currentLocation, item.port);
}

function enrichShipmentsWithTemperature(shipments, temperature) {
  const temps = temperatureMap(temperature);
  return shipments.map(item => {
    const temp = temperatureForShipment(temps, item);
    const missingTemperature = item.status === 'overseasTransit' && !temp;
    const shouldUseTemperature = item.status === 'overseasTransit' && !!temp;
    return {
      ...item,
      location: locationForShipment(item, temp),
      temperatureStatus: shouldUseTemperature ? temp.status : (missingTemperature ? 'MISSING' : ''),
      temperatureStatusText: shouldUseTemperature ? temp.statusText : (missingTemperature ? TEMP_STATUS_TEXT.MISSING : ''),
      missingTemperature,
    };
  });
}
...
function buildTemperature(logistics) {
  ...
  // 按唯一柜号分组，避免同一柜多条温度记录被重复计算
  const byContainer = new Map();
  withDeviation.forEach(item => {
    const cNo = item.containerNo || '-';
    if (!byContainer.has(cNo)) byContainer.set(cNo, []);
    byContainer.get(cNo).push(item);
  });
  const totalContainers = byContainer.size;
  const alarmContainers = Array.from(byContainer.values())
    .filter(records => records.some(r => r.status === 'ALARM')).length;
  const warningContainers = Array.from(byContainer.values())
    .filter(records => records.some(r => r.status === 'WARN' && !records.some(r2 => r2.status === 'ALARM'))).length;
  ...
  return {
    ...
    alarmCount: alarmContainers,
    warningCount: warningContainers,
    alarmRate: pct(alarmContainers, totalContainers),
  ...
  rows.forEach(item => {
    ...
    if (!grouped.has(key)) {
      grouped.set(key, {
        ...
        brand: item.brand,
        status: item.status,
        statusText: item.statusText || TEMP_STATUS_TEXT[item.status] || item.status,
        records: [],
      });
    }
    const row = grouped.get(key);
    if (item.status === 'MISSING') row.missing = item;
    else row.records.push(item);
    if (item.status === 'ALARM' || (item.status === 'WARN' && row.status !== 'ALARM')) {
      row.status = item.status;
      row.statusText = item.statusText || TEMP_STATUS_TEXT[item.status] || item.status;
    }
  });
  return {
    days,
    rows: Array.from(grouped.values()).slice(0, 8).map(row => ({
      ...
      cells: days.map(day => {
        const records = row.records
          .filter(item => dateKey(item.recordedAt || item.releaseDate) === day.key)
          .sort((a, b) => (parseTimeMs(a.recordedAt) || 0) - (parseTimeMs(b.recordedAt) || 0));
        const segments = records.map(item => ({
          level: item.status === 'ALARM' ? 'alarm' : item.status === 'WARN' ? 'warn' : 'ok',
        ...
        const missing = row.missing && day.key === days[days.length - 1].key;
        const active = segments.length > 0 || missing;
        const worst = segments.some(segment => segment.level === 'alarm')
          ? 'alarm'
          : segments.some(segment => segment.level === 'warn')
            ? 'warn'
            : segments.length
              ? 'ok'
              : missing
        ...
        return {
          date: day.key,
          level: active ? worst : 'none',
          segments,
          returnTemp: segments[segments.length - 1] ? segments[segments.length - 1].returnTemp : null,
          setTemp: segments[segments.length - 1] ? segments[segments.length - 1].setTemp : null,
          deviation: segments[segments.length - 1] ? segments[segments.length - 1].deviation : null,
          statusText: missing ? TEMP_STATUS_TEXT.MISSING : segments.map(segment => segment.statusText).filter(Boolean).join(' / '),
        };
      }),
    })),
  };
...
function buildTransitTemperatureGantt(shipments, temperature) {
  const temps = temperatureMap(temperature);
  const matchedTempKeys = new Set();
  const rows = shipments
    .filter(item => item.status === 'overseasTransit')
    .map(item => {
      const records = temperaturesForShipment(temps, item);
      if (records.length) {
        const key = matchKey(records[0].containerNo, records[0].releaseDate);
        if (key) matchedTempKeys.add(key);
        return records;
      }
      return {
        containerNo: firstContainer(item),
        ...
        deviation: null,
        status: 'MISSING',
        statusText: TEMP_STATUS_TEXT.MISSING,
    ...
    .flat();
  (temperature.details || [])
    .filter(item => isOverseasTemperature(item))
    .filter(item => {
      const key = matchKey(item.containerNo, item.releaseDate);
      return key ? !matchedTempKeys.has(key) : !rows.some(row => row.containerNo === item.containerNo);
    })
  ...
  return buildTemperatureGantt(rows);
}

function buildRisks(flowAlerts, temperature, aggregate, shipments = []) {
  ...
  shipments
    .filter(item => item.status === 'overseasTransit' && item.missingTemperature)
    .forEach(item => {
      const raw = normalizeShipment(item);
      risks.push({
        type: '温度缺失',
        ...
        days: num(item.dwellDays),
        statusText: raw.statusText,
        summary: '国外在途柜未匹配到温度记录，需要补录或核查温度表',
        severity: 'high',
        raw,
      });
    });
...
function buildDrilldowns({ orderRows, shipments, bottlenecks, risks, temperature, structure }) {
  ...
  return {
    ...
    shipped: { title: '已发货柜明细', columns: ['containerNo', 'orderNo', 'country', 'factory', 'category', 'shipDate', 'statusText', 'location', 'temperatureStatusText', 'dwellDays'], rows: normalizedShipments },
    arrived: { title: '已到岸柜明细', columns: ['containerNo', 'orderNo', 'country', 'factory', 'arrivalDate', 'statusText', 'location', 'dwellDays'], rows: byStatus('arrived') },
    ...
    bottlenecks: Object.fromEntries(bottlenecks.map(item => [item.key, {
      title: `${item.label}明细`,
      columns: ['containerNo', 'orderNo', 'country', 'factory', 'category', 'statusText', 'location', 'temperatureStatusText', 'dwellDays'],
    ...
    byCountry: Object.fromEntries((structure.country || []).map(item => [item.label, {
      title: `${item.label}明细`,
      columns: ['containerNo', 'orderNo', 'country', 'factory', 'category', 'statusText', 'location', 'dwellDays'],
    ...
    byContainer: Object.fromEntries(normalizedShipments.map(row => [row.containerNo, {
      ...
      columns: ['containerNo', 'orderNo', 'country', 'factory', 'category', 'shipDate', 'arrivalDate', 'domesticShipDate', 'signedDate', 'statusText', 'location', 'temperatureStatusText', 'dwellDays'],
      rows: [row],
    }])),
  };
}
...
function buildExecutiveOverview({ aggregate = {}, flow = {}, filters = {} }) {
  ...
  const filteredShipments = enrichShipmentsWithTemperature(all.filter(item => matchesFilters(item, activeFilters)), temperature);
  temperature.gantt = buildTransitTemperatureGantt(filteredShipments, temperature);
  ...
  const risks = buildRisks(filteredFlowAlerts, temperature, aggregate, filteredShipments);
