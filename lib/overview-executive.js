...
function temperatureMap(temperature) {
  return new Map((temperature.details || []).map(item => [item.containerNo, item]));
}
...
function enrichShipmentsWithTemperature(shipments, temperature) {
  ...
  return shipments.map(item => {
    const containerNo = firstContainer(item);
    const temp = temps.get(containerNo);
...
function buildTransitTemperatureGantt(shipments, temperature) {
  ...
  const rows = shipments
    ...
    .map(item => {
      const temp = temps.get(firstContainer(item));
      if (temp) return temp;
      return {
        containerNo: firstContainer(item),
        brand: item.brand || factoryOf(item) || '-',
