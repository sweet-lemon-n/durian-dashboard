...
function normalizeTemperature(item) {
  ...
  return {
    ...
    port: item.port || '',
    releaseDate: item.releaseDateText || item.placementTimeText || item.releaseTimeText || dateKey(item.releaseDate || item.placementTime || item.releaseTime),
...
function temperatureMap(temperature) {
  ...
  (temperature.details || []).forEach(item => {
    ...
    if (key && !byShipmentDate.has(key)) byShipmentDate.set(key, item);
  });
  ...
  return {
    ...
    rows: rows.slice(0, 8).map(item => ({
      ...
      cells: days.map((day, idx) => {
        const activeDate = item.status === 'MISSING'
          ? days[days.length - 1].key
          : dateKey(item.recordedAt || item.releaseDate);
        const active = activeDate && day.key === activeDate;
        return {
          date: day.key,
          level: active
          ? (item.status === 'MISSING' ? 'missing' : item.status === 'ALARM' ? 'alarm' : item.status === 'WARN' ? 'warn' : 'ok')
          : 'none',
          returnTemp: active ? item.returnTemp : null,
          setTemp: active ? item.setTemp : null,
          deviation: active ? item.deviation : null,
          statusText: active ? (item.statusText || TEMP_STATUS_TEXT[item.status] || item.status) : '',
        };
      }),
    })),
  };
...
function buildTransitTemperatureGantt(shipments, temperature) {
  ...
  const rows = shipments
    ...
    .map(item => {
      const temp = temperatureForShipment(temps, item);
      if (temp) {
        const key = matchKey(temp.containerNo, temp.releaseDate);
