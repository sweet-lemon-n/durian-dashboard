...
function normalizeShipment(item) {
  return {
    ...
    location: item.location || item.currentLocation || item.port || item.arrivalPort || '',
...
function locationForShipment(item, temp) {
  if (item.status === 'overseasTransit') {
    return (temp && temp.location) || item.location || item.currentLocation || item.port || '';
  }
  if (item.status === 'onShore') {
    return item.port || item.arrivalPort || item.location || item.currentLocation || '';
  }
  return item.location || item.currentLocation || item.port || '';
}
...
function enrichShipmentsWithTemperature(shipments, temperature) {
  ...
  return shipments.map(item => {
    ...
    return {
      ...
      temperatureStatus: temp ? temp.status : (missingTemperature ? 'MISSING' : ''),
      temperatureStatusText: temp ? temp.statusText : (missingTemperature ? '温度缺失' : ''),
      missingTemperature,
    };
  });
}
