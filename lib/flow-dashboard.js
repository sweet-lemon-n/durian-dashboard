...
function reduceShipment(shipDate, records, fields, context = {}) {
  ...
  const linkIds = linkedRecordIds(first, fields.detailOrderNo);
  const linkedOrderNo = linkIds.map(id => context.orderNoByRecordId && context.orderNoByRecordId.get(id)).find(Boolean);
  const orderNo = linkedOrderNo || textValue(first, fields.detailOrderNo);
...
function buildShipments(detailRecords, fields, context = {}) {
  ...
  detailRecords.forEach(record => {
    const linkIds = linkedRecordIds(record, fields.detailOrderNo);
    const linkedOrderNo = linkIds.map(id => context.orderNoByRecordId && context.orderNoByRecordId.get(id)).find(Boolean);
    const orderNo = linkedOrderNo || textValue(record, fields.detailOrderNo);
...
function buildFlowFacts(orderRecords, detailRecords, orderFields, detailFields) {
  const orderFacts = [];
  const orderCountryByNo = new Map();
  ...
  orderRecords.forEach(record => {
    ...
    if (orderNo && record.record_id) orderNoByRecordId.set(String(record.record_id), orderNo);
    if (orderNo && country) orderCountryByNo.set(orderNo, country);
  ...
  const shipments = buildShipments(detailRecords, detailFields, { orderNoByRecordId });
  shipments.forEach(item => {
    if (!item.country && orderCountryByNo.has(item.orderNo)) item.country = orderCountryByNo.get(item.orderNo);
  });
