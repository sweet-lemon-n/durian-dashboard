...
const DEFAULT_FLOW_DASHBOARD_CONFIG = {
  orderSheetTitle: '订单主表',
  detailSheetTitle: '分柜明细表',
  orderSheetId: '',
  detailSheetId: '',
  fields: {
    orderNo: { fieldId: '', title: '订单编号' },
    freshBoxes: { fieldId: '', title: '鲜果(柜数)' },
    frozenBoxes: { fieldId: '', title: '冻果(柜数)' },
    detailOrderNo: { fieldId: '', title: '订单编号' },
    containerNo: { fieldId: '', title: '柜号' },
    shipDate: { fieldId: '', title: '发货日期' },
    arrivalDate: { fieldId: '', title: '到岸时间' },
    domesticShipDate: { fieldId: '', title: '国内发货时间' },
    signedDate: { fieldId: '', title: '签收日期' },
    country: { fieldId: '', title: '国家' },
    category: { fieldId: '', title: '产品品类' },
    brand: { fieldId: '', title: '品牌' },
  },
  thresholds: {
    overseasTransitDays: 7,
    onShoreDays: 2,
    domesticTransitDays: 2,
  },
};
...
function normalizeFlowField(value, fallback) {
  const v = value && typeof value === 'object' ? value : {};
  return {
    fieldId: String(v.fieldId || '').trim(),
    title: String(v.title || fallback.title || '').trim(),
  };
}

function normalizeFlowDashboardConfig(input = {}) {
  const prev = input && typeof input === 'object' ? input : {};
  const defaults = DEFAULT_FLOW_DASHBOARD_CONFIG;
  const fields = {};
  Object.entries(defaults.fields).forEach(([key, fallback]) => {
    fields[key] = normalizeFlowField(prev.fields && prev.fields[key], fallback);
  });
  return {
    orderSheetTitle: String(prev.orderSheetTitle || defaults.orderSheetTitle).trim(),
    detailSheetTitle: String(prev.detailSheetTitle || defaults.detailSheetTitle).trim(),
    orderSheetId: String(prev.orderSheetId || '').trim(),
    detailSheetId: String(prev.detailSheetId || '').trim(),
    fields,
    thresholds: {
      overseasTransitDays: Math.max(0, Number(prev.thresholds?.overseasTransitDays ?? defaults.thresholds.overseasTransitDays) || 0),
      onShoreDays: Math.max(0, Number(prev.thresholds?.onShoreDays ?? defaults.thresholds.onShoreDays) || 0),
      domesticTransitDays: Math.max(0, Number(prev.thresholds?.domesticTransitDays ?? defaults.thresholds.domesticTransitDays) || 0),
    },
  };
}

function getFlowDashboardConfig() {
  const cfg = readRuntimeConfig();
  return normalizeFlowDashboardConfig(cfg.flowDashboard || {});
}

function getPublicFlowDashboardConfig() {
  return getFlowDashboardConfig();
}

function updateFlowDashboardConfig(input) {
  const cfg = readRuntimeConfig();
  cfg.flowDashboard = {
    ...normalizeFlowDashboardConfig(input || {}),
    updatedAt: new Date().toISOString(),
  };
  writeRuntimeConfig(cfg);
  return getPublicFlowDashboardConfig();
}
