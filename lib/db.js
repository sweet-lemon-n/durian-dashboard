...
function normalizeDashboardPermissions(permissions) {
  let list = permissions;
  if (list === undefined || list === null) return DASHBOARD_SECTIONS.slice();
  if (typeof permissions === 'string') {
    try { list = JSON.parse(permissions); } catch (_) { list = DASHBOARD_SECTIONS.slice(); }
  }
  if (!Array.isArray(list)) list = DASHBOARD_SECTIONS.slice();
  return Array.from(new Set(list.filter(p => DASHBOARD_SECTIONS.includes(p))));
}

function stringifyDashboardPermissions(permissions) {
  return JSON.stringify(normalizeDashboardPermissions(permissions));
}
...
function initDatabase(dbPath) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      dashboard_permissions TEXT NOT NULL DEFAULT '["summary","th","vn","logistics","news","gantt"]',
  ...
  if (!userColumns.includes('dashboard_permissions')) {
    db.prepare("ALTER TABLE users ADD COLUMN dashboard_permissions TEXT NOT NULL DEFAULT '[\"summary\",\"th\",\"vn\",\"logistics\",\"news\",\"gantt\"]'").run();
  }
 ...
 */
async function createUser(database, { username, password, displayName, role, permissions, dashboardPermissions }) {
  ...
  const stmt = database.prepare(
    'INSERT INTO users (username, password_hash, display_name, role, permissions, dashboard_permissions) VALUES (?, ?, ?, ?, ?, ?)'
  );
  const result = stmt.run(
    ...
    stringifyDashboardPermissions(dashboardPermissions)
  );
...
function updateUser(database, id, fields) {
  const allowed = ['password_hash', 'display_name', 'role', 'is_active', 'token_version', 'permissions', 'dashboard_permissions'];
  const sets = [];
  const values = [];
...
function listUsers(database) {
  return database.prepare(
    'SELECT id, username, display_name, role, is_active, token_version, permissions, dashboard_permissions, created_at, updated_at FROM users ORDER BY id'
  ).all().map(user => ({
    ...user,
