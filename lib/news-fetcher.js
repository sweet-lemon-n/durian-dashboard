...
async function fetchFromMetasoMcp() {
  const apiKey = process.env.METASO_MCP_API_KEY || process.env.METASO_API_KEY;
  const mcpUrl = process.env.METASO_MCP_URL;
  ...
  const all = [];
  const queries = SEARCH_KEYWORDS.map(kw => `${kw} 最新 新闻`);
  const calls = await Promise.allSettled(queries.map((q, i) =>
    postJsonRpc(mcpUrl, apiKey, 'tools/call', {
      name: tool.name,
      arguments: buildToolArgs(tool, q),
    }, 100 + i)
  ));

  for (const r of calls) {
    if (r.status !== 'fulfilled') {
      console.warn('[news-fetcher] 秘塔 MCP tools/call 失败:', r.reason && r.reason.message);
      continue;
    }
    normalizeSearchItems(r.value)
      .map(item => normalizeNewsItem(item, 'metaso-mcp'))
      .filter(Boolean)
...
async function fetchFromMetasoApi() {
  const apiKey = process.env.METASO_API_KEY;
  const apiUrl = process.env.METASO_API_URL;
  ...
  const results = await Promise.allSettled(queries.map(async (q) => {
    ...
    return normalizeSearchItems(resp.data)
      .map(item => normalizeNewsItem(item, 'metaso'))
      .filter(Boolean);
  }));

  for (const r of results) {
    if (r.status === 'fulfilled') all.push(...r.value);
    else console.warn('[news-fetcher] 秘塔搜索失败:', r.reason && r.reason.message);
  }
  return all;
}

async function fetchFromMetaso() {
  const mcpItems = await fetchFromMetasoMcp();
  if (mcpItems.length) return mcpItems;
  return fetchFromMetasoApi();
}
...
async function fetchAllSina() {
  const all = [];
  const results = await Promise.allSettled(SINA_LIDS.map(l => fetchSinaFeed(l.lid, 60)));
  for (const r of results) {
    if (r.status === 'fulfilled') all.push(...r.value);
  }
  return all;
  ...
  for (const kw of SEARCH_KEYWORDS) {
    try {
      ...
      for (let i = 0; i < Math.min(titles.length, 4); i++) {
        ...
        if (!isRelevantNews(t.title, detail)) continue;
        all.push({
...
async function refreshInternal() {
  ...
  // 0. 秘塔搜索（配置后优先）
  try {
    const metasoItems = await fetchFromMetaso();
...
module.exports = { initNewsFetcher, stopNewsFetcher, getAutoNews, getLastFetchedTime, getNewsSourceSummary, refreshNow };
