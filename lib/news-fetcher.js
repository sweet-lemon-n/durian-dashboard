...
async function fetchFromMetasoApi() {
  ...
  normalizeSearchItems(resp.data)
...
async function refreshInternal() {
  ...
  try {
    const cfg = getNewsSourceConfig();
    let metasoItems = [];
    if (cfg.metasoEnabled) {
      metasoItems = await fetchFromMetaso();
    } else {
      noteDiag(diag, 'metaso', 'skipped', '秘塔未启用，已跳过付费搜索源');
    }
