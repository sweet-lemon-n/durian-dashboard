 ...
 *
 * 源优先级：
 *   1. 秘塔搜索 API（可选，配置 METASO_API_KEY + METASO_API_URL）
...
async function fetchFromMetaso() {
  ...
  const results = await Promise.allSettled(queries.map(async (q) => {
    ...
    return normalizeSearchItems(resp.data).map(item => {
      const title = stripHtml(item.title || item.name || item.headline || item.summary);
