...
async function loadLogistics(docid, sheetList) {
  ...
  const activeContainerSet = new Set(overseasShipments.map(s => s.containerNo).filter(Boolean));
  ...
  const activeTempList = latestList.filter(r => activeContainerSet.has(wecom.getRecordValue(r, '柜号')));
