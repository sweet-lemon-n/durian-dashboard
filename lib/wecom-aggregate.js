...
async function loadLogistics(docid, sheetList) {
  ...
  // 温度统计：同一柜号可多次装柜，按柜号 + 放柜日期保留每趟最新一条
  ...
  tempRecs.forEach(r => {
    ...
    const cur = latestByTrip.get(key);
    if (!cur || t > cur._t) latestByTrip.set(key, { _t: t, raw: r });
  });
  const latestList = Array.from(latestByTrip.values()).map(x => x.raw);
  const activeTempList = latestList.filter(r => {
    const key = shipmentTemperatureKey(wecom.getRecordValue(r, '柜号'), wecom.getRecordValue(r, '放柜时间'));
