 ...
 *   - delivered (互斥版) = 分柜明细 COUNT(到岸时间非空 AND 签收日期空)
 *   - transit = 分柜明细 COUNT(发货日期非空 AND 到岸时间空)
 *   - pending = boxes − 该(订单,品类)在分柜明细中出现的柜数
 *   - port = 0（企微数据没有"在口岸"维度）
 *
 *   - TH 模式 rate = (signed+delivered)/boxes，前端 delivered 段含签收
 *   - VN 模式 rate = signed/boxes，delivered 段为"到岸但未签收"
 *
...
async function loadOrders(docid, sheetList) {
  ...
  orderRecs.forEach(o => {
    ...
    [['FRESH', fresh, '鲜果'], ['FROZEN', frozen, '冻肉']].forEach(([cat, boxes, label]) => {
      ...
      // 到岸但未签收（互斥）→ 让前端 signed+delivered 段不会把同一柜数加两遍
      const arrivedNotSigned = dCat.filter(d => d.arr && !d.sign).length;
      // 已发货但未到岸
      ...
      rows.push({
        ...
        boxes,
        signed,
        delivered: arrivedNotSigned, // 已交付（到岸但未签收）—— 看板里 signed+delivered 段加起来不重复
