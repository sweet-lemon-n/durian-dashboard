...
function seedData() {
  return {
    ...
    orders: [
      ...
      { id: 'TH-YG-FRESH',  country: 'TH', category: 'FRESH',  brand: 'YG 鲜果',        orders: 1, boxes: 0,  signed: 0, delivered: 0,  transit: 0, port: 0, pending: 0  },
...
function ensureDefaultRows(data) {
  let changed = false;
  if (Array.isArray(data.orders)) {
    const hasYgFresh = data.orders.some(o =>
      o && o.country === 'TH' && o.category === 'FRESH' && /^YG\b/i.test(String(o.brand || ''))
    );
    if (!hasYgFresh) {
      const idx = data.orders.findIndex(o => o && o.id === 'TH-KK-FROZEN');
      data.orders.splice(idx >= 0 ? idx : data.orders.length, 0, {
        id: 'TH-YG-FRESH',
        country: 'TH',
        category: 'FRESH',
        brand: 'YG 鲜果',
...
function read() {
  ...
  try {
    ...
    if (ensureDefaultRows(data)) writeSync(data);
    return data;
  } catch (e) {
