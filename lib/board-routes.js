...
const { buildExecutiveOverview } = require('./overview-executive');
const wecomCache = require('./wecom-cache');
const { normalizeDashboardPermissions, getDb } = require('./db');
const {
  saveOverviewSnapshot,
  getOverviewSnapshotComparison,
  dateKey: overviewSnapshotDateKey,
...
router.get('/overview-executive', async (req, res) => {
  try {
    const snapshot = await wecomCache.getSnapshotOrRefresh(process.env.DOCID);
    const aggregate = applyDashboardVisibility(await aggregateFromWecom(process.env.DOCID, snapshot), req.user);
    if (aggregate.news) {
      aggregate.news.th = [];
      aggregate.news.vn = [];
      aggregate.news.auto = getAutoNews();
      aggregate.news.fetchedAt = getLastFetchedTime();
    }
    ...
    try {
      const hasFilters = Boolean(req.query.country || req.query.factory || req.query.container);
      const db = getDb();
      if (!hasFilters) {
        saveOverviewSnapshot(db, {
          snapshotDate: overviewSnapshotDateKey(),
          totalOrders: overview.headline.totalOrders,
          totalBoxes: overview.headline.totalBoxes,
          shipped: overview.headline.shipped,
          signed: overview.headline.signed,
          totalRisks: overview.headline.totalRisks,
          healthScore: overview.headline.healthScore,
        });
      }
      overview.snapshotComparisons = getOverviewSnapshotComparison(db, overviewSnapshotDateKey(), 7);
    } catch (snapshotError) {
      console.warn('[overview-executive] KPI 快照写入/读取失败:', snapshotError.message);
      overview.snapshotComparisons = null;
    }
    res.set('Cache-Control', 'no-store');
