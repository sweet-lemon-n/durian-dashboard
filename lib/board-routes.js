...
const { getAutoNews, refreshNow } = require('./news-fetcher');
...
function aggregate(db) {
  ...
  return {
    ...
    news: { th: db.news.filter(n => n.country === 'TH'), vn: db.news.filter(n => n.country === 'VN'), auto: getAutoNews() },
...
router.get('/aggregate', async (req, res) => {
  ...
  if (currentSource === 'wecom') {
    try {
      ...
      if (out.news) out.news.auto = getAutoNews();
      return res.json(out);
    } catch (e) {
...
router.post('/news/auto/refresh', adminOnly, async (req, res) => {
  try {
    const db = read();
    const aggData = aggregate(db);
    await refreshNow(aggData);
